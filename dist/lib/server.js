...
class Server extends _eventemitter.default {
  ...
  constructor(options) {
    ...
    this.wss.on("connection", (socket, request) => {
      ...
      const u = _url.default.parse(request.url, true);
      ...
      if (!this.namespaces[ns]) this._generateNamespace(ns); // store socket and method
  ...
  of(name) {
    ...
    return {
      ...
      connected() {
        const socket_ids = [...self.namespaces[name].clients.keys()];
        return socket_ids.reduce((acc, curr) => Object.assign(Object.assign({}, acc), {
  ...
  _handleRPC(socket, ns = "/") {
    socket.on("message", async data => {
      ...
      if (Array.isArray(parsedData)) {
        if (!parsedData.length) return socket.send(JSON.stringify({
          jsonrpc: "2.0",
        ...
        return socket.send(_circularJson.default.stringify(responses), msg_options);
      }

      const response = await this._runMethod(parsedData, socket._id, ns);
      if (!response) return;
      return socket.send(_circularJson.default.stringify(response), msg_options);
    });
  }
  /**
  ...
  async _runMethod(message, socket_id, ns = "/") {
    ...
    let response = null; // reject request if method is protected and if client is not authenticated
    ...
    if (!message.id) return; // if login middleware returned true, set connection as authenticated
